...
The `main.workflow` is a simple text file, and it is reasonable human-readable.
...
This is meant to get your started, or to help you apply the same `main.workflow` to many similar projects that you might have.
...
However, leveraging this isolation requires some understanding of Docker, the open-source industry standard by which these separate computing environments are defined.
...
For many other R projects, such as some R markdown project, there is *no* such standard computing environment, and you actually *want* to have the flexibility to provision one for yourself (say, because you need some version of a system dependency such as the GSL or LaTeX).
...
- It seems pretty fast compared to TravisCI: an `R CMD check` for a small project with all bells and whistles is at <4 minutes. (This is anecdotal evidence, if you'd like a real benchmarking, chip in [here](https://github.com/maxheld83/ghactions/issues/35)).
...
<!-- TODO: mention that GitHub opened the spec -->

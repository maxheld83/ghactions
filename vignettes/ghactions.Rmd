...
Every GitHub *action* (not just the entire workflow) runs in its own little virtual machine (but thinner) called a Docker container.
If you already know what this is, skip ahead.
If you're not familiar with Docker, it's easy to learn.

A Docker container (loosely speaking) exists in several forms.
...
Using the `docker build` shell command, you can then, well, *build* a container *image*, which gets stored somewhere on your computer and which you can share with others.
...
Some GitHub actions use one of the many publicly available images to get their work done, such as, for example, the [action to publish to GitHub Pages](https://github.com/maxheld83/ghpages).
...
Because containers can be layered, you can just base your *own* `Dockerfile` on of those popular images, such as `rocker/verse`, which includes R, RStudio, the tidyverse, LaTeX and many other packages and system dependencies.
...
Most workflow function in this package start with a block such as the below, added via `ghactions::docker_cli()`.
...
Actions are the parts that make up a workflow.
In other CI/CD services, these are sometimes known as *steps*, though GitHub actions are more flexible.

The below workflow function(s) paste together individual actions and make them work together.
...
- There's one less service to deal with and one less authentication to go through, especially for smaller projects. No more PATs.
